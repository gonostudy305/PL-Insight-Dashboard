...
@Component({
    ...
    <div class="alerts-page">
      <h2 class="page-title">🚨 Hệ thống cảnh báo</h2>
      ...
      <div class="alert-list">
        @for (alert of alerts; track alert._id) {
          <div class="alert-card" [class]="'priority-' + alert.priority_level">
            <div class="alert-priority-bar"></div>
            <div class="alert-body">
              <div class="alert-header">
                <span class="priority-badge">{{ alert.priority_label }}</span>
                <span class="alert-stars">
                  @for (s of getStarsArray(alert.stars); track s) { ⭐ }
                </span>
                <span class="alert-branch">📍 {{ alert.branch_address }}</span>
              </div>
              <p class="alert-text">{{ alert.text }}</p>
              <div class="alert-tags">
                @for (factor of alert.risk_factors; track factor) {
                  <span class="risk-tag">{{ factor }}</span>
                }
              </div>
              <div class="alert-meta">
                <span>🕐 {{ alert.session }}</span>
                <span>📝 {{ alert.text_length_group }}</span>
              </div>
            </div>
          </div>
        }
